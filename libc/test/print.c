...
static void print_llx(unsigned long long ullInteger)
{
    int strlen = 0;
    int inx = 0;
    ...
    while (ullInteger) {
        remain = ullInteger & 0xf;
        str[strlen++] = hex_digit_char(remain);
        ullInteger >>= 4;
    }
    for (; inx < strlen; inx++) {
        putchar(str[strlen - 1 - inx]);
    }
}
                ...
                case 'l':
                    switch (*(++c)) {
                        case 'l':
                            switch (*(++c)) {
                                case 'x':
                                    ullInteger = va_arg(arg_ptr, unsigned long long);
                                    print_llx(ullInteger);
                                    break;
                                default:
                                    putchar('%');
                                    putchar('l');
